...
def generate_context_state():
    ...
    # Determine output path - prefer .claude/ directory if it exists
    cwd = Path.cwd()
    claude_dir = cwd / '.claude'

    # Try to use .claude/ directory, fall back to project root
    if claude_dir.exists() and claude_dir.is_dir():
        output_file = claude_dir / 'CONTEXT_STATE.md'
        location = ".claude/"
    else:
        output_file = cwd / 'CONTEXT_STATE.md'
...
- Check CONTINUE_WORK.md for task tracking (auto-updated on Stop hook)
- Use PROJECT_INDEX.json for architectural awareness
- File saved to: `{output_file.relative_to(cwd) if output_file != cwd / 'CONTEXT_STATE.md' else 'CONTEXT_STATE.md'}`
...
"""
    ...
    try:
        with open(output_file, 'w') as f:
            f.write(content)
        print(f"✓ Generated CONTEXT_STATE.md in {location}")
        return True
